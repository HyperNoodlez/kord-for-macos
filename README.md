...
# 1. Build the Rust core library (two-pass build, handles ggml symbol dedup)
./build-rust.sh
...
The first build compiles Whisper and llama.cpp from source — this takes several minutes. Subsequent builds are incremental and much faster.

### First Launch

1. Kord appears in your menu bar as a small swirl icon
2. Click the icon and choose **Show Settings**
3. Go to **Transcription** → download a Whisper model (start with `base`)
4. Go to **LLM** → download an LLM model (start with `Qwen2.5 1.5B` for speed or `Qwen3 4B` for quality)
5. Press `Alt+Space` and start talking
...
│          Rust kord-core                 │
│  ┌─────────┐ ┌───────┐ ┌───────────┐   │
│  │ whisper- │ │ llama-│ │  SQLite   │   │
│  │ rs (C++) │ │ cpp-2 │ │  History  │   │
│  └─────────┘ └───────┘ └───────────┘   │
│  ┌─────────┐ ┌───────┐ ┌───────────┐   │
│  │  cpal   │ │ enigo │ │  arboard  │   │
│  │ (audio) │ │(keybd)│ │(clipboard)│   │
│  └─────────┘ └───────┘ └───────────┘   │
└─────────────────────────────────────────┘
```

- **Frontend**: SwiftUI with `NavigationSplitView`, custom `NSPanel` overlay, Carbon hotkey API
- **Backend**: Rust static library exposing C FFI functions
- **Bridge**: All complex types serialized as JSON strings across the FFI boundary
- **Audio**: CoreAudio via `cpal` with real-time resampling to 16kHz for Whisper
- **Storage**: SQLite database at `~/Library/Application Support/com.labgarge.kord/`
...
├── build-rust.sh                    # Two-pass Rust build script
├── Sources/
│   ├── CKordCore/                   # C module map for FFI
│   └── KordNative/
│       ├── KordNativeApp.swift      # App entry point, menu bar setup
│       ├── ContentView.swift        # Main settings window
│       ├── Models/                  # AppSettings, HistoryEntry
│       ├── Services/                # KordCore FFI wrapper, audio bridge, hotkeys
│       ├── Theme/                   # Accent colors, spacing constants
│       └── Views/
│           ├── RecordingOverlay*.swift   # Floating overlay panel + view
│           ├── Components/              # Reusable UI: ModeCard, HotkeyRecorder
│           └── Settings/               # All settings tab views
├── kord-core/                       # Rust library
│   ├── Cargo.toml
│   ├── include/kord_core.h          # Generated C header (committed)
│   └── src/
│       ├── ffi.rs                   # C FFI exports
│       ├── audio/                   # Recording, VAD, resampling, visualization
│       ├── transcription/           # Whisper engine, model manager
│       ├── llm/                     # LLM client, prompt templates, model manager
│       ├── clipboard/               # Paste methods via enigo + arboard
│       ├── db.rs                    # SQLite history storage
│       └── settings.rs             # Settings persistence
└── visual assets/                   # App logo, company logo, menu bar icon
```
